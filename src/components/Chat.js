...
// 		initial2: {
// 			type: 'choice',
// 			message: () => queryAi(messages),
// 			options: ['לומד', 'עובד', 'משחק', 'בכלל לא עושה כלום'].map((state) => ({
// 				label: state,
// 				next: 'initial3'
// 			}))
// 		},
...
// 	const addUserMessage = async (payload, options = {}) => {
// 		setInputClassName();
// 		setMessages((messages) => [...messages, { payload, role: 'user', ...options }]);
// 		await fetchBotReply();
// 	};

// 	const addBotReply = (payload, options = {}) => {

// 		const timer = setTimeout(() => setTyping(true), (Math.random() * 500) + 500);

// 		setTimeout(() => {
// 			clearTyping();
// 			addMessage(data.message, { role: 'assistant' });
// 		}, 500);
...
// 				method: 'POST',
// 				body: JSON.stringify(messages.map(({ message, role }) => ({ role, content: message }))),
// 				headers: {
...
// 	useEffect(() => {
// 		if (!messages.length) setMessages([messageTypes.initial]);
// 	}, [messages]);
...
// 					return <Message model={messagify(message)} key={index} className={isRtl(message?.message) ? 'is-rtl' : ''} />;
// 				})}
// 			</MessageList>
...
// 	function messagify(message) {
// 		return {
// 			role: message.role,
// 			type: message.type === 'choice' ? 'custom' : 'text',
// 			payload: message.type === 'question' ? message : <Message.CustomContent>
// 				{message.options.map((option) => <div key={option} className="box" onClick={setUserChoice}>{option}</div>)}
// 			</Message.CustomContent>
// 		};
// 	}

// 	async function setUserChoice(event) {
// 		const messageText = event.currentTarget.innerText;
// 		await (lastMessage.callback?.(messageText) || Promise.resolve());
// 		addMessage(messageText, { role: 'user' });
// 	}
